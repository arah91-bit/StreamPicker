...
CATALOG = [
    ...
    ("CACHE_REVERIFY_TTFB", "fast", "num", "8", _S,
     "First-byte deadline for revalidating the top cached stream."),
    ...
    ("HLS_SEG_CACHE_MB", "proxy", "num", "64", "",
     ...
     "ahead of the player)."),
    ("HLS_SEG_CACHE_TTL_SECONDS", "proxy", "num", "300", _S,
     "Maximum age of an HLS segment in the in-memory read-ahead cache."),
    ("HLS_BUFFER_CONCURRENCY", "proxy", "num", "4", "",
     "Maximum HLS prefetch/body-buffer operations running concurrently."),
    ...
    ("PROXY_SESSION_MAX", "proxy", "num", "5000", "",
     "Hard cap on live playback capability sessions retained in memory."),
    ...
    ("NZB_LANE_MAX_ACTIVE", "usenet", "num", "32", "",
     "Maximum concurrently active direct-usenet lane operations."),
    ("NZB_LANE_REGISTRY_MAX", "usenet", "num", "500", "",
     "Hard cap on remembered direct-usenet lane entries."),
    ...
    ("ACQUIRE_FOREGROUND_WAIT", "acquire", "num", "3", _S,
     "Seconds to wait for an add request before returning a checking notice."),
    ...
    ("TB_AUTO_CACHE", "acquire", "bool", "1", "",
     ...
     "own debrid config) so the title plays cached on a later visit. On by "
     "default — TorBox's cache is global, so every cache helps every TorBox "
     "user. Set 0 to disable."),
    ("TB_CACHE_MAX_SLOTS", "acquire", "num", "2", "",
     "How many of the plan's TORBOX_MAX_DOWNLOADS download slots auto-cache "
     "may occupy at once; the rest always stay free for your own activity. "
     "Never exceeds the plan limit."),
